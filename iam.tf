...
## private_ec2_2
resource "aws_iam_role" "private_ec2_2" {
  name               = "${var.project_name}-${var.environment}-private_ec2_2-iamrole"
  assume_role_policy = data.aws_iam_policy_document.ec2_assume_role.json
}

resource "aws_iam_role_policy_attachment" "private_ec2_2_ssm" {
  role       = aws_iam_role.private_ec2_2.name
  policy_arn = data.aws_iam_policy.systems_manager.arn
}

resource "aws_iam_role_policy_attachment" "private_ec2_2_cloudwatch" {
  role       = aws_iam_role.private_ec2_2.name
  policy_arn = data.aws_iam_policy.cloudwatch_agent.arn
}

resource "aws_iam_instance_profile" "private_ec2_2" {
  name = "${var.project_name}-${var.environment}-private_ec2-2-instanceprofile"
  role = aws_iam_role.private_ec2_2.name
}